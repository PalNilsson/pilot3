...
class ErrorCodes:
    ...
    def is_looping_error(self, codes: list) -> bool:
        """Return True if any of the given error codes means a looping payload.

        There is more than one looping code - the plain one and the variant for
        a node whose CVMFS could not be read - and callers that guard on
        "was this a looping job" have to honour all of them. Keeping the set in
        one place stops a new code from silently changing behaviour in the
        experiment plugins, where the guard decides whether the payload's
        working directory is preserved for analysis.

        Args:
            codes: Pilot error codes reported for the job.

        Returns:
            True if any of them is a looping error code.
        """
        looping_codes = (self.LOOPINGJOB, self.LOOPINGJOBCVMFS)

        return any(code in (codes or []) for code in looping_codes)
